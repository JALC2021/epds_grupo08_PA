...
include 'conexion.php';
$sql = "select * from pago where pago_restante = '0'";
$result = mysql_query($sql,$conexion);
while(($fila = mysql_fetch_row($result)) != null){
	...
	<tr class="gradeA">
		<?php for($i=0;$i<8;$i++){?>
			...
			$sql1 = "select foto_usuario from socios where id_usuario = '$fila[1]'"; 
			$result1 = mysql_query($sql1,$conexion);

			$fila1 = mysql_fetch_array($result1);
			?>

			<td> <img src=" <?php  echo "fotos/".$fila1[0]; ?> " width = "60px"/> </td>	
			<td> 
				<form method="post" action="eliminarPagoRealizado.php"><input type="hidden" name="idPago" value="<?php echo $fila[0]; ?> "><button type="submit" name="delete">Borrar</button></form>
				<form method="post" action="pages-invoice.php"><input type="hidden" name="idPago" value="<?php echo $fila[0]; ?> "><input type="hidden" name="idUsuario" value="<?php echo $fila[1]; ?> "><button type="submit" name="factura">Factura</button></form>
					...
					<header class="page-header">
						...
						<div class="right-wrapper pull-right">
							<ol class="breadcrumbs">
								...
								<li><span>Gestionar Pagos</span></li>
								<li><span>Pagos Realizados</span></li>
							</ol>

...
function sacarPagoRestante($id){
	include 'conexion.php';
	$sql = "select pago_restante from pago where id='".$id."'";
	$result = mysql_query($sql,$conexion);
	$fila = mysql_fetch_row($result);
	mysql_close($conexion);
	return $fila[0];
}
function sacarPagoRealizado($id){
	include 'conexion.php';
	$sql = "select pago_realizado from pago where id='".$id."'";
	$result = mysql_query($sql,$conexion);
	$fila = mysql_fetch_row($result);
	mysql_close($conexion);
	return $fila[0];
...
$_SESSION['pagoRestante'] = sacarPagoRestante($_SESSION['idpago']);
$_SESSION['pagoRealizado'] = sacarPagoRealizado($_SESSION['idpago']);
}
...
if(isset($_POST['editarPago'])){
	...
	if($_POST['añadirPago'] > $_SESSION['pagoRestante']) {
		$fails[] = "El pago añadido del socio es incorrecto no debe superar el pago restante";
	}else{
		$datos['pagoRealizado'] = filter_var(filter_input(INPUT_POST, 'añadirPago',FILTER_SANITIZE_NUMBER_INT));
		$datos['pagoRealizado'] += $_SESSION['pagoRealizado'];
		$datos['pagoRestante'] = ($_SESSION['pagoRestante'] - $_POST['añadirPago']);
	}


	if(isset($fails)){
		echo "<li><p class = 'm-none text-semibold h6'>".$fails[0]."</p></li>";
	}else{
		insertar($datos);
	}
...
function insertar($datos){
		...
		include 'conexion.php';
		$sql = "update pago set pago_realizado ='".$datos['pagoRealizado']."', pago_restante = '".$datos['pagoRestante']."' where id = '".$_SESSION['idpago']."'";
		mysql_query($sql,$conexion);
		mysql_close($conexion);
		header("Location:pagoPendiente.php");
}
                                    ...
                                    <form class="form-horizontal form-bordered" action="#" method="post"> 
                                       <input type = "hidden" name = "socio" value = "<?php echo $idSocio; ?>">
  										 <div class="form-group">
                                            <label class="col-md-3 control-label">Pago Restante</label>
                                            <div class="col-md-6">
                                                <div data-plugin-spinner>
                                                    <div class="input-group input-small">
                                                        <input type="text" name = "pagoTotal" class="spinner-input form-control" value ="<?php echo $_SESSION['pagoRestante'] ?>" >
                                         ...
                                         <div class="form-group">
                                            <label class="col-md-3 control-label">Añadir pago</label>
                                            <div class="col-md-6">
                                                <div data-plugin-spinner>
                                                    <div class="input-group input-small">
                                                        <input type="text" name = "añadirPago" class="spinner-input form-control" >
                                                        <div class="spinner-buttons input-group-btn btn-group-vertical">
                                                            <button type="button" class="btn spinner-up btn-xs btn-default">
                                                                <i class="fa fa-angle-up"></i>
                                                            </button>
                                        ...
                                        <div class="form-group" align="center">
                                            <button type="submit" class="mb-xs mt-xs mr-xs btn btn-warning btn-lg" value="enviar" name="editarPago">Enviar</button>

...
include 'conexion.php';
$sql = "select id,nombre,descripcion,duracion,precio from tarifa";
$result = mysql_query($sql,$conexion);
while(($fila = mysql_fetch_row($result)) != null){
	...
	<tr class="gradeA">
		<?php for($i=0;$i<5;$i++){?>
			...
			<td> 
				<form method="post" action="eliminarTarifa.php"><input type="hidden" name="idTarifa" value="<?php echo $fila[0]; ?> "><button type="submit" name="delete">Borrar</button></form>
				<form method="post" action="editarTarifa.php"><input type="hidden" name="idTarifa" value="<?php echo $fila[0]; ?> "><button type="submit" name="editar">Editar</button></form>

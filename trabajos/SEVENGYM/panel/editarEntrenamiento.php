...
if(isset($_POST['editarEntrenamiento'])){
	
	if(!isset($_POST['tipoEntrenamiento']) or $_POST['tipoEntrenamiento'] == ""){
		$fails[] = "El tipo de entrenamiento es incorrecto";
	}else{
		$datos['tipoEntrenamiento'] = filter_var(filter_input(INPUT_POST, 'tipoEntrenamiento',FILTER_SANITIZE_SPECIAL_CHARS));
	}

	if(!isset($_POST['nombreEntrenamiento']) or $_POST['nombreEntrenamiento'] == ""){
		$fails[] = "El nombre del entrenamiento es incorrecto";
	}else{
		$datos['nombreEntrenamiento'] = filter_var(filter_input(INPUT_POST, 'nombreEntrenamiento', FILTER_SANITIZE_SPECIAL_CHARS));
	}

	if(!isset($_POST['duracion']) or $_POST['duracion'] == "" or !preg_match('/^[0-9]*$/', $_POST['duracion'] )){
		$fails[] = "La duracion es incorrecta";
	}else{
		$datos['duracion'] = filter_var(filter_input(INPUT_POST, 'duracion',FILTER_SANITIZE_NUMBER_INT));
	}
	if(!isset($_POST['numEjercicios']) or $_POST['numEjercicios'] == "" or !preg_match('/^[0-9]*$/', $_POST['numEjercicios'] ) or $_POST['numEjercicios']<0){
		$fails[] = "El numero de ejercicios es incorrecto";
	}else{
		$datos['numEjercicios'] = filter_var(filter_input(INPUT_POST, 'numEjercicios',FILTER_SANITIZE_NUMBER_INT));
	}

	if(!isset($_POST['tipoDieta']) or $_POST['tipoDieta'] == ""){
		$fails[] = "El tipo de dieta es incorrecto";
	}else{
		$datos['tipoDieta'] = filter_var(filter_input(INPUT_POST, 'tipoDieta', FILTER_SANITIZE_SPECIAL_CHARS));
	}

	if(!isset($_POST['observaciones']) or $_POST['observaciones'] == ""){
		$fails[] = "La observacion es incorrecta";
	}else{
		$datos['observaciones'] = filter_var(filter_input(INPUT_POST, 'observaciones', FILTER_SANITIZE_SPECIAL_CHARS));
	}

		if(!isset($_POST['intensidad']) or $_POST['intensidad'] == "" or !preg_match('/^[0-9]*$/', $_POST['intensidad'] )){
		$fails[] = "La intensidad es incorrecta";
	}else{
		$datos['intensidad'] = filter_var(filter_input(INPUT_POST, 'intensidad',FILTER_SANITIZE_NUMBER_INT));
	}
...
function actualizar($datos){
	include 'conexion.php';
		$sql = "update entrenamiento set tipo='".$datos['tipoEntrenamiento']."',nombre='".$datos['nombreEntrenamiento']."',duracion=".$datos['duracion'].",num_ejercicio=".$datos['numEjercicios'].",tipo_dieta='".$datos['tipoDieta']."',observacion='".$datos['observaciones']."',intensidad=".$datos['intensidad']." where id = ".$datos['idEntren']." and id_entrenador=".$_SESSION['id']."  ";
...
<title>Panel de Administración Seven Gym</title>
	...
	<body>
		...
		echo "</ul></div>";
					...
					<?php if($_SESSION['rol'] == "monitor" and isset($_POST['editar'])){ ?>
                    <div class="row">
                        <div class="col-xs-12">
                            <section class="panel">
                                ...
                                <div class="panel-body">
                                    <form class="form-horizontal form-bordered" action="#" method="post" enctype = "multipart/form-data"> 
                                        <div class="form-group">
                                            <label class="col-md-3 control-label" for="inputDefault">Tipo</label>
                                            <div class="col-md-6">
                                                <input type="text" class="form-control" id="inputDefault" name = "tipoEntrenamiento">
                                            </div>
                                        </div>
                                        <div class="form-group">
                                            <label class="col-md-3 control-label" for="inputDefault">Nombre</label>
                                            <div class="col-md-6">
                                                <input type="text" class="form-control" id="inputDefault" name = "nombreEntrenamiento">
                                            </div>
                                        </div>
                                          <div class="form-group">
                                            <label class="col-md-3 control-label">Duracion (min)</label>
                                            <div class="col-md-6">
                                                <div data-plugin-spinner>
                                                    <div class="input-group input-small">
                                                        <input type="text" name = "duracion" class="spinner-input form-control" >
                                           ...
                                           <div class="form-group">
                                            <label class="col-md-3 control-label">Numero de Ejercicios</label>
                                            <div class="col-md-6">
                                                <div data-plugin-spinner>
                                                    <div class="input-group input-small">
                                                        <input type="text" name = "numEjercicios" class="spinner-input form-control" >
                                                        <div class="spinner-buttons input-group-btn btn-group-vertical">
                                                            <button type="button" class="btn spinner-up btn-xs btn-default">
                                                                <i class="fa fa-angle-up"></i>
                                                            </button>
                                        ...
                                        <div class="form-group">
                                            <label class="col-md-3 control-label" for="inputDefault">Tipo de dieta</label>
                                            <div class="col-md-6">
                                                <input type="text" class="form-control" id="inputDefault" name = "tipoDieta">
                                            </div>
                                        </div>
                                        ...
                                        <div class="form-group">
                                         ...
                                         <div class="form-group">
                                            <label class="col-md-3 control-label">Intensidad</label>
                                            <div class="col-md-6">
                                                <div class="m-md slider-primary1" data-plugin-slider data-plugin-options='{ "value": 5, "range": "min", "max": 10 }' data-plugin-slider-output="#listenSlider">
                                                    <input id="listenSlider" name = "intensidad" type="hidden" value="50" />
                                                </div>
                                                <p class="output">La <code>intensidad</code> es de: <b>5</b> puntos</p>
                                            </div>
                                        </div>                                        

                                        <div class="form-group" align="center">
                                        	<input type = "hidden" name= "idEntren" value= "<?php echo $_POST['idEntrenamiento'] ?>">
                                            <button type="submit" class="mb-xs mt-xs mr-xs btn btn-warning btn-lg" value="enviar" name="editarEntrenamiento">Enviar</button>

                        ...
                        if (!$result) {
                            ...
                            if (mysqli_num_rows($result) > 0) {
                                while ($row = mysqli_fetch_array($result)) {
                                    ?>
                                    <li><a class="studentlink" href="studentView.php?studentId=<?php echo $row['id']; ?>">
                                            <?php
                                            echo ucwords($row['firstName']) . " " . ucwords($row['lastName']);
                                            if ($row['active'] == 'N') {
                                                ?><a class="studentlinklogo" href="activeStudent.php?subjectId=<?php echo $subjectId; ?>&studentToActiveId=<?php echo $row['id'] ?>"><img class="acceptlogo" src="img/accept.png" /></a><?php
                                            }
                                            ?>
                                        </a>
                                    </li>
